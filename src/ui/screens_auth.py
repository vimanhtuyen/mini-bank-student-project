...
from src.ui.ui_helpers import is_pin_format_valid, read_non_negative_integer
...
class RegisterFrame(ttk.Frame):
    def __init__(self, parent, app):
        ...
        ttk.Label(self, text='Mở tài khoản mới', style='HeaderTitle.TLabel').pack(anchor='w', pady=(0, 14))
        ...
        form_card = ttk.Frame(body, style='Card.TFrame', padding=(24, 20))
        form_card.grid(row=0, column=0, sticky='nsew', padx=(0, 10))
        ttk.Label(form_card, text='Biểu mẫu đăng ký', style='SectionTitle.TLabel').pack(anchor='w')
        ttk.Label(
            form_card,
        ...
        self.register_feedback_label = ttk.Label(form_card, text='Nhập thông tin để bắt đầu.', style='Muted.TLabel')
        self.register_feedback_label.pack(anchor='w', pady=(14, 8))

        preview_box = ttk.LabelFrame(form_card, text='Xem trước', style='Card.TLabelframe')
        ...
        ttk.Button(button_row, text='Tạo tài khoản', command=self.create_account, style='Primary.TButton').grid(row=0, column=0, padx=(0, 10))
        ttk.Button(button_row, text='Làm trống', command=self.clear_inputs, style='Secondary.TButton').grid(row=0, column=1, padx=(0, 10))
        ttk.Button(button_row, text='Quay lại', command=self.go_back, style='Light.TButton').grid(row=0, column=2)

        right_card = ttk.Frame(body, style='Card.TFrame', padding=(24, 20))
        right_card.grid(row=0, column=1, sticky='nsew')
        ttk.Label(right_card, text='Hướng dẫn nhanh', style='SectionTitle.TLabel').pack(anchor='w')
        for text in [
            ...
            ttk.Label(right_card, text=text, style='Surface.TLabel', wraplength=320, justify='left').pack(anchor='w', pady=4)

        ttk.Label(right_card, text='Mô phỏng giống ứng dụng thật', style='CardTitle.TLabel').pack(anchor='w', pady=(16, 4))
        ttk.Label(
            right_card,
    ...
    def create_account(self) -> None:
        ...
        if owner_name == '':
            messagebox.showwarning('Lỗi nhập liệu', 'Tên chủ tài khoản không được để trống.')
            return
...
class LoginFrame(ttk.Frame):
    def __init__(self, parent, app):
        ...
        ttk.Label(self, text='Đăng nhập tài khoản', style='HeaderTitle.TLabel').pack(anchor='w', pady=(0, 14))
        ...
        form_card = ttk.Frame(body, style='Card.TFrame', padding=(24, 20))
        form_card.grid(row=0, column=0, sticky='nsew', padx=(0, 10))
        ttk.Label(form_card, text='Xác thực người dùng', style='SectionTitle.TLabel').pack(anchor='w')
        ttk.Label(
            form_card,
        ...
        self.login_feedback_label = ttk.Label(form_card, text='Nhập thông tin để tiếp tục.', style='Muted.TLabel')
        self.login_feedback_label.pack(anchor='w', pady=(14, 8))
        self.recent_account_label = ttk.Label(form_card, text='Tài khoản gần nhất: chưa có dữ liệu.', style='Strong.TLabel')
        self.recent_account_label.pack(anchor='w', pady=(0, 16))

        button_row = ttk.Frame(form_card, style='Card.TFrame')
        button_row.pack(anchor='w')
        ttk.Button(button_row, text='Đăng nhập', command=self.login, style='Primary.TButton').grid(row=0, column=0, padx=(0, 10))
        ttk.Button(button_row, text='Dùng tài khoản vừa tạo', command=self.prefill_recent_account, style='Secondary.TButton').grid(row=0, column=1, padx=(0, 10))
        ttk.Button(button_row, text='Quay lại', command=self.go_back, style='Light.TButton').grid(row=0, column=2)

        right_card = ttk.Frame(body, style='Card.TFrame', padding=(24, 20))
        right_card.grid(row=0, column=1, sticky='nsew')
        ttk.Label(right_card, text='Tại sao giao diện này dễ dạy?', style='SectionTitle.TLabel').pack(anchor='w')
        ttk.Label(
            right_card,
    ...
    def prefill_recent_account(self) -> None:
        ...
        if recent_account_id == '':
            messagebox.showinfo('Thông báo', 'Chưa có tài khoản vừa tạo trong phiên làm việc này.')
            return
    ...
    def login(self) -> None:
        ...
        pin_code = self.pin_code_entry.get().strip()
        if account_id == '':
            messagebox.showwarning('Lỗi nhập liệu', 'Số tài khoản không được để trống.')
            return
        if pin_code == '':
            messagebox.showwarning('Lỗi nhập liệu', 'PIN không được để trống.')
            return
